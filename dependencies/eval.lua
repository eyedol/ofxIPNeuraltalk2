...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    for k=1,#sents do
      ...
      if verbose then
        -- Hack to dump caption to a file
	local cmd = 'echo ' .. entry.caption .. '> ~/description.txt'
	-- Hack to push  caption to a text to speech app
	local voiceCmd = '~/voice.sh "'  .. entry.caption .. '"'
	print(cmd)
	print(voiceCmd)
	os.execute(cmd)
	os.execute(voiceCmd);
